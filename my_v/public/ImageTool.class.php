...
/**
 * +------------------------------------------------------------------------------
 * 图像操作类库（依赖StringTool.class.php）
 *
 * 工具函数：
 *
 * getImageInfo 取得图像信息
 * thumb              生成缩略图
 * buildString      根据给定的字符串生成图像
 * buildImageVerify 生成图像验证码（并放入session）
 * GBVerify           中文验证码
 * showASCIIImg  把图像转换成字符显示
 * showAdvVerify  生成高级图像验证码（并放入session）
 * UPCA                   生成UPC-A条形码
 * output               输出图像到浏览器（一般为内部调用）
 * buildImageByPostByte 二进制post数据生成图片（未经测试）
 *
 */
class ImageTool { //类定义开始
	...
	+----------------------------------------------------------
	 */
	static function buildString( $string, $width = 100, $height = 30, $rgb = array(), $filename = '', $type = 'png', $disturb = 1, $border = true ) {
		if ( $type != 'gif' && function_exists( 'imagecreatetruecolor' ) ) {
...
//        include_once('/StringTool.class.php');
...
//        @imagestring($im, 5, 5, 3, $randval, $stringColor);
	 ...
	 */
	static function GBVerify( $length = 4, $type = 'png', $width = 180, $height = 50, $fontface = 'calibri.ttf', $verifyName = 'verify' ) {
//        include_once("/StringTool.class.php");
	 ...
	 * @param $path
	 */
	static function buildImageByPostByte($fileName, $path) {
		$xmlstr =  $GLOBALS['HTTP_RAW_POST_DATA'];                        // 获取post数据

		if(empty($xmlstr)) {                                                                             // 如果post数据为空，之后？？？
