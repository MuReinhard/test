 ...
 * format 设置格式
 * isLeapYear 是否是闰年
 * firstDayOfMonth 计算月份的第一天
 * firstDayOfYear 计算年份的第一天
 * lastDayOfMonth 计算月份的最后一天
 * lastDayOfYear 计算年份的最后一天
 * maxDayOfMonth 计算月份的最大天数
 * dateAdd 取得指定增加（年，月，日，时，分）后的日期
 * numberToCh 日期数字转中文
 * yearToCh   年份数字转中文
 * magicInfo 判断日期 所属 干支 生肖 星座
 * dateDiff 人性化的计算日期差
 */
class DateUtil { //类定义开始
	...
	public function firstDayOfMonth() {
		return ( new DateUtil( strftime( "%Y-%m-%d", mktime( 0, 0, 0,
			$this->month,
			1,
			$this->year ) ) ) );
	}
	...
	public function maxDayOfMonth() {
		$result = $this->dateDiffProcess( strtotime( $this->dateAdd( 1, 'm' ) ), 'd' );
		return $result;
	}
	...
	public function dateAdd( $number = 0, $interval = "d" ) {
		...
		return ( new DateUtil( mktime( $hours,
			$minutes,
			$seconds,
			$month,
			$day,
	...
	+----------------------------------------------------------
	 */
	public function dateDiff( $time, $precision = false ) {
		if ( ! is_numeric( $precision ) && ! is_bool( $precision ) ) {
			static $_diff = array('y' => '年', 'M' => '个月', 'd' => '天', 'w' => '周', 's' => '秒', 'h' => '小时', 'm' => '分钟');
			return ceil( $this->dateDiffProcess( $time, $precision ) ) . $_diff[$precision];
		}
	...
	+----------------------------------------------------------
	 */
	private function dateDiffProcess( $date, $elaps = "d" ) {
		$__DAYS_PER_WEEK__ = ( 7 );
